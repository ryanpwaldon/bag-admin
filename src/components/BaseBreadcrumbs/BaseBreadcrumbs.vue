  ...
  <div v-if="breadcrumbs.length > 1">
    <nav class="sm:hidden">
      <router-link
        ...
        class="flex items-center text-sm font-medium leading-5 text-gray-400 transition duration-150 ease-in-out hover:text-gray-700"
      >
    ...
    <nav class="items-center hidden text-sm font-medium leading-5 sm:flex">
      <template v-for="(route, i) in breadcrumbs" :key="i">
        <router-link :to="{ name: route.name }" class="text-gray-400 transition duration-150 ease-in-out hover:text-gray-700">
          {{ route.meta.title }}
        </router-link>
