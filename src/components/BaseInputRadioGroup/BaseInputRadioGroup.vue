    ...
    <ul class="relative -space-y-px bg-white rounded-md">
      <li v-for="(option, i) in options" :key="i">
        ...
        >
          ...
          <p class="pl-1 ml-6 text-sm leading-5 md:ml-0 md:pl-0 md:text-center" v-if="option.meta1 || option.meta2">
            <span :class="modelValue === option.value ? 'text-blue-900' : 'text-gray-900'" class="font-medium" v-if="option.meta1">
              {{ option.meta1 }}
            </span>
            <span :class="modelValue === option.value ? 'text-blue-700' : 'text-gray-500'" v-if="option.meta2">
              {{ option.meta2 }}
            </span>
          </p>
          <p
            ...
            :class="[modelValue === option.value ? 'text-blue-700' : 'text-gray-500', option.meta1 || option.meta2 ? '' : 'col-span-2']"
          >
            {{ option.meta3 }}
          </p>
        </div>
