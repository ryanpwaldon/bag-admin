<template>
  <BaseGridCard :content-padding="false" class="overflow-hidden">
    ...
    <BaseTable :loading="loading" :items="items" :handle-selection="handleSelection" :props="properties" v-if="loading || items.length">
      ...
      <template #total="{ item }">
        <div class="text-sm leading-5 text-gray-500">{{ format.currency(item.order.details.total_price) }}</div>
      </template>
      <template #link>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const items = ref([] as Conversion<ProgressBar>[])
    const properties = [
      { name: 'Order', id: 'order' },
    ...
    const fetchItems = async () => {
      ...
      const response = await conversionService.findByOffer({
        offerId: props.progressBar.id,
        conversionType: ConversionType.ProgressBar,
        page: page.value,
        limit: limit.value,
        sort: '-createdAt'
      })
